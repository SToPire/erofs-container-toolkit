...
func (s *Snapshotter) Prepare(ctx context.Context, key, parent string, opts ...snapshots.Opt) ([]mount.Mount, error) {
	...
	manifestDigest, layerIndex, err := s.resolveProxyTarget(ctx, parent, labels)
	if err != nil {
		return nil, err
	}
	if manifestDigest == "" {
...
func (s *Snapshotter) resolveProxyTarget(ctx context.Context, parent string, labels map[string]string) (digest.Digest, int, error) {
	if manifestText := labels[converter.EROFSManifestAnnotation]; manifestText != "" {
		dgst, err := digest.Parse(manifestText)
		if err != nil {
			return "", 0, fmt.Errorf("parse %s: %w", converter.EROFSManifestAnnotation, err)
		}
		return dgst, 0, nil
	}
	...
	dgst, err := digest.Parse(manifestText)
	if err != nil {
		return "", 0, fmt.Errorf("parse parent proxy manifest digest: %w", err)
	}
	layerIndex, err := strconv.Atoi(layerIndexText)
	if err != nil {
		return "", 0, fmt.Errorf("parse parent proxy layer index: %w", err)
	}

	return dgst, layerIndex + 1, nil
}

func (s *Snapshotter) layerBlobPath(snapshotID string) string {
	if s.root == "" || snapshotID == "" {
		return ""
	}
	return filepath.Join(s.root, "snapshots", snapshotID, "layer.erofs")
}
